...
# deny: git mutations, destructive removes, secret/env writes, .github edits, outbound network
if printf '%s' "$cmd" | grep -Eq 'git[[:space:]]+(push|commit|reset|rebase|checkout)|rm[[:space:]]+-rf|>[[:space:]]*\.env|tee[[:space:]]+\.env|\.github/|(^|[[:space:]])(curl|wget)[[:space:]]'; then
  printf '{"permission":"deny","user_message":"Blocked by Pigment governance hook.","agent_message":"Denied: git mutations, rm -rf, .env writes, .github edits, and outbound network are not allowed. Stay within Edit(app|components|lib) + npm test/typecheck/build. A denial means re-scope, not retry."}'
  exit 0
fi
