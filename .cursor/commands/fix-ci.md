...
2. Identify the **root cause**, not the symptom. For an a11y-gate failure, that's almost
   always a token in `components/ui/status-badge.tsx` (or `app/globals.css`) that dropped below
   WCAG AA — fix the token to a value that clears the threshold and stays on-brand.
